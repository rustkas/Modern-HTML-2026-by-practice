**Глава 16. Declarative Shadow DOM (DSD)**

### Главная идея главы

**Declarative Shadow DOM** позволяет описывать Shadow DOM прямо в HTML с помощью атрибута `shadowrootmode` у элемента `<template>`. Это особенно важно для:

- SSR и гибридных архитектур;
- инкапсуляции стилей и разметки без обязательного JavaScript при первой отрисовке;
- подготовки к настоящим Web Components.

Если раньше Shadow DOM создавался только императивно через JavaScript, то теперь браузер может получить уже готовое shadow-дерево из HTML.
...
<my-element>
  <template shadowrootmode="open">
    <style>/* инкапсулированные стили */</style>
    <p>Содержимое shadow-дерева</p>
    <slot></slot>
  </template>
</my-element>
```
...
Основные режимы:
- `shadowrootmode="open"` — shadow root доступен из JavaScript
- `shadowrootmode="closed"` — более строгая инкапсуляция

### Практическая задача главы

1. Создать переиспользуемый блок «Карточка задачи» с Declarative Shadow DOM.
2. Инкапсулировать часть стилей карточки внутри shadow-дерева.
3. Показать использование `<slot>` для проекции внешнего контента.
4. Сохранить работоспособность страницы даже если JS ещё не выполнился.

### Структура проекта после Главы 16
...
Структура файлов пока не расширяется — компонент описываем прямо в `index.html` (в следующих главах вынесем в отдельные файлы/Custom Elements).
  ...
  <main id="app-main">
    ...
    <section id="tasks" class="app-section" data-section-panel="tasks">
      ...
      <section aria-labelledby="active-tasks-heading">
        ...
        <!-- Карточка задачи на Declarative Shadow DOM -->
        ...
        <!-- Вторая задача пока оставлена в обычной разметке для сравнения -->
...
/* Стили обычных task-карточек (для сравнения с DSD-версией) */
.task {
  border: 1px solid #e2e8f0;
  border-radius: 12px;
  background: #fff;
...
- Первая карточка задачи переведена на Declarative Shadow DOM
- Использован `<template shadowrootmode="open">`
- Добавлены слоты: `title`, `media`, `description`, `priority-label`, `actions`
- Часть стилей карточки инкапсулирована внутри shadow-дерева
- Показан progressive путь к Web Components без обязательного JS при первой отрисовке
...
1. Откройте страницу.
2. Сравните первую задачу (DSD) и вторую задачу (обычная разметка).
3. В DevTools посмотрите shadow root у `<task-card>`.
4. Отключите JavaScript — декларативная shadow-разметка первой карточки должна сохраниться.

## Следующие шаги

**Часть 6. Web Components**

- Глава 17. Custom Elements
- Превращение `<task-card>` в полноценный пользовательский элемент
```
...
- Declarative Shadow DOM полезен уже сейчас, даже до регистрации Custom Element.
- Слоты позволяют сохранить гибкость: снаружи передаём контент, внутри контролируем структуру и стили.
- Это отличный мост к полноценным Web Components и SSR.

В следующей главе мы сделаем следующий шаг: зарегистрируем настоящий **Custom Element** и начнём собирать компонентную архитектуру FocusBoard.